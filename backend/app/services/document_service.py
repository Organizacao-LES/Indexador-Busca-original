...
class DocumentService:
    def __init__(self, document_repository: DocumentRepository):
        ...
        self.allowed_extensions = {extension.lower() for extension in settings.DOCUMENT_ALLOWED_EXTENSIONS}
    ...
    def upload_document(
        ...
        *,
        ...
        extracted_content = self._extract_content_preview(extension, content)
    ...
    def to_history_response(self, payload: dict) -> dict:
        ...
        details = (
            f"Validado ({payload['type']}) • {self._format_size(payload['size_bytes'])} • "
    ...
    def to_details_response(self, payload: dict) -> dict:
        document_date = payload["document_date"].isoformat() if payload["document_date"] else payload["uploaded_at"].isoformat()
        return {
            "id": payload["id"],
            "title": payload["title"],
            "category": payload["category"],
    ...
    def _register_invalid_document(self, db: Session, uploaded_by: User, filename: str, reason: str) -> None:
    ...
    def _validate_extension(self, extension: str) -> None:
        if extension not in self.allowed_extensions:
            ...
            raise DocumentValidationException(f"Tipo de arquivo inválido. Formatos aceitos: {allowed}.")
    ...
    def _validate_integrity(self, extension: str, content: bytes) -> None:
        ...
        if extension in {"txt", "csv"}:
            try:
                ...
                raise DocumentValidationException("Arquivo de texto inválido: codificação UTF-8 obrigatória.") from exc
    ...
    def _extract_content_preview(self, extension: str, content: bytes) -> str | None:
        if extension in {"txt", "csv"}:
            return content.decode("utf-8")[:4000]
        return "Documento PDF recebido e validado. Extração textual ainda não foi executada."
